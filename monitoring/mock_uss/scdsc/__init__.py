...
if KEY_INTERACTIONS_LOG_DIR in webapp.config:
    logger.debug(f"KEY_INTERACTIONS_LOG_DIR - {KEY_INTERACTIONS_LOG_DIR} is in config")
    ...
    from monitoring.mock_uss.interaction_logging.logger import (
        log_flask_interaction as log,
    )
else:
    logger.debug(f"KEY_INTERACTIONS_LOG_DIR - {KEY_INTERACTIONS_LOG_DIR} not in config")
