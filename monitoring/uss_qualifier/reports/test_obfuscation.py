...
def test_obfuscate_directory():
    ...
    with tempfile.TemporaryDirectory() as in_dir, tempfile.TemporaryDirectory() as out_dir:
        ...
        # Verify content
        with open(os.path.join(out_dir, "gate3", "participant1.html"), "r") as f:
            obf_html = f.read()
        ...
        assert obf_json["report"]["queries"][0]["request"]["url"] == "http://host1/dss/v1"
        assert obf_json["report"]["queries"][0]["request"]["headers"]["Authorization"] == "Bearer REDACTED"
