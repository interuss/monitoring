    ...
    def _validate_mock_uss_notifications(self, notifications_received_after: datetime):
        ...
        for test_flight in self._injected_flights:
            with self.check(
                ...
                # As a conservative approximation of the time when a notification should be sent out, we use the time of completion of the injection query.
                latency = min(notif_reception_times) - (
                    test_flight.query_timestamp
                    + timedelta(seconds=test_flight.query_duration_s)
                )
