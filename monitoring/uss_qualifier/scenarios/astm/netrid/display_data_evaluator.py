...
def map_fetched_to_injected_flights(
    ...
    :param query_cache: A opaque dict, used to maintain participant_id in various queries
    ...
    # Init caching object if needed
    if "mappings" not in query_cache:
        query_cache["mappings"] = {}

    if "unmaped" not in query_cache:
        query_cache["unmaped"] = []
    ...
    for mapping in tel_mapping.values():
        if mapping.observed_flight.flights_url not in query_cache["mappings"]:
            query_cache["mappings"][
                mapping.observed_flight.flights_url
    ...
    for uss_query in fetched_flights:
        if not uss_query.has_field_with_value("participant_id"):
            # print("NO PID", uss_query)
            query_cache["unmaped"].append(uss_query)

    # Try to map existing queries
    for query_not_mapped in query_cache["unmaped"]:
    ...
    query_cache["unmaped"] = list(
        filter(lambda q: not q.participant_id, query_cache["unmaped"])
    )
