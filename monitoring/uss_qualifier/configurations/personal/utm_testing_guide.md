...
*   **U-Space Flight Authorization**
    ...
    *   *(Note: Verify exact path in `suites/uspace` if using)*

## 2. How to Test Your UTM Service

To test your service, you need to create a **Configuration File** that tells `uss_qualifier` two things:
1.  **What to test:** Which test suite to run.
2.  **What to test against:** The URL and authentication details of *your* service.
          ...
          specification:
            flight_planners:
              - participant_id: my_service_under_test
                # UPDATE THIS URL to point to your service
                v1_base_url: http://host.docker.internal:3000/flight_planning/v1
...
docker run --rm -v $(pwd):/app/monitoring \
  ...
  interuss/monitoring \
  uv run monitoring/uss_qualifier/main.py --config configurations.personal.my_utm_test
```
...
*   **Scenarios:** Individual test logic (e.g., "Nominal Planning", "Conflict with Higher Priority").

## 4. Next Steps
1.  Navigate to `monitoring/uss_qualifier/configurations/dev` to see complex examples like `f3548_self_contained.yaml`.
2.  Copy a relevant example to `configurations/personal/`.
3.  Modify the `base_url` fields to point to your local or staging service.
4.  Run the test and check the `report.html` generated in `monitoring/uss_qualifier/output`.
