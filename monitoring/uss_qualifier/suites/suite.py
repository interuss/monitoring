...
class TestSuiteAction[T: ActionGenerator]:
    ...
    def __init__(
        ...
        resources_for_child = make_child_resources(
            ...
            f"Test suite action to run {action} {action.get_child_type()}",
        )
    ...
    def run(self, context: ExecutionContext) -> TestSuiteActionReport:
        ...
        if skip_report:
            logger.warning(
                f"Skipping {self.declaration} '{self.get_name()}' because: {skip_report.reason}"
            )
    ...
    def __init__(
        ...
        for a, action_dec in enumerate(self.definition.actions):
            try:
                ...
                logger.warning(
                    f"Skipping action {a} ({action_dec} {action_dec.get_child_type()}) because {str(e)}"
                )
                actions.append(
                    SkippedActionReport(
...
class ExecutionContext:
    ...
    def end_action(
        ...
        if self.current_frame.action is not action:
            raise RuntimeError(
                f"Action {self.current_frame.action.declaration} {self.current_frame.action.declaration.get_child_type()} was started, but a different action {action.declaration} {action.declaration.get_child_type()} was ended"
            )
