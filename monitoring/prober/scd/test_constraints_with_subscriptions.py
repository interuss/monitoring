...
def test_create_constraint(ids, scd_api, scd_session, scd_session2):
    ...
    assert SUB_BASE_URL_A not in [
        subscriber["uss_base_url"] for subscriber in subscribers
    ], subscribers
    subscriberb = [
        subscriber
        for subscriber in subscribers
        if subscriber["uss_base_url"] == SUB_BASE_URL_B
    ]
    assert len(subscriberb) == 1, subscribers
    subscriberb = subscriberb[0]
    assert ids(SUB2_TYPE) in [
        subscription["subscription_id"] for subscription in subscriberb["subscriptions"]
    ]
    assert ids(SUB3_TYPE) in [
        subscription["subscription_id"] for subscription in subscriberb["subscriptions"]
    ]
    sub2_index = [
...
def test_mutate_constraint(ids, scd_api, scd_session, scd_session2):
    ...
    if scd_api == scd.API_0_3_17:
        resp = scd_session.put(
            "/constraint_references/{}/{}".format(
                ids(CONSTRAINT_TYPE), existing_constraint["ovn"]
            ),
            json=req,
            scope=SCOPE_CM,
        )
...
def test_mutate_subs(ids, scd_api, scd_session2, scd_session):
    ...
    if scd_api == scd.API_0_3_17:
        resp = scd_session2.put(
            "/subscriptions/{}/{}".format(ids(SUB1_TYPE), existing_sub["version"]),
            json=req,
            scope=_read_both_scope(scd_api),
        )
    ...
    assert ids(CONSTRAINT_TYPE) in [
        constraint["id"] for constraint in data.get(key, [])
    ], data
    ...
    if scd_api == scd.API_0_3_17:
        resp = scd_session2.put(
            "/subscriptions/{}/{}".format(ids(SUB3_TYPE), existing_sub["version"]),
            json=req,
            scope=_read_both_scope(scd_api),
        )
    else:
...
def test_mutate_constraint2(ids, scd_api, scd_session, scd_session2):
    ...
    subscribera = [
        subscriber
        for subscriber in subscribers
        if subscriber["uss_base_url"] == SUB_BASE_URL_A
    ]
    assert len(subscribera) == 1, subscribers
    subscribera = subscribera[0]
    subscribera_subscriptions = [
        subscription["subscription_id"] for subscription in subscribera["subscriptions"]
    ]
