...
def _make_op_request(idx):
    ...
    return {
        "extents": [
            scd.make_vol4(time_start, time_end, 0, 120, scd.make_circle(lat, 178, 50))
        ],
        "old_version": 0,
        "state": "Accepted",
    ...
    resp = scd_session.post(
        ...
        json={
            "area_of_interest": scd.make_vol4(
                time_now, end_time, 0, 5000, scd.make_circle(-56, 178, 12000)
            )
        },
        scope=SCOPE_SC,
    )
...
def test_get_ops_by_search(ids, scd_api, scd_session):
    resp = scd_session.post(
        ...
        json={
            "area_of_interest": scd.make_vol4(
                None, None, 0, 5000, scd.make_circle(-56, 178, 12000)
            )
        },
    )
    assert resp.status_code == 200, resp.content
...
def test_get_ops_by_search_earliest_time_included(ids, scd_api, scd_session):
    ...
    resp = scd_session.post(
        ...
        json={
            "area_of_interest": scd.make_vol4(
                earliest_time, None, 0, 5000, scd.make_circle(-56, 178, 12000)
            )
        },
    )
    assert resp.status_code == 200, resp.content
...
def test_get_ops_by_search_latest_time_included(ids, scd_api, scd_session):
    ...
    resp = scd_session.post(
        ...
        json={
            "area_of_interest": scd.make_vol4(
                None, latest_time, 0, 5000, scd.make_circle(-56, 178, 12000)
            )
        },
    )
    assert resp.status_code == 200, resp.content
...
def test_mutate_ops(ids, scd_api, scd_session):
    for idx, op_id in enumerate(map(ids, OP_TYPES)):
        ...
        resp = scd_session.post(
            "/operational_intent_references/query",
            json={"area_of_interest": req["extents"][0]},
        )
        assert resp.status_code == 200, resp.content
        found_ids = [
            op["id"] for op in resp.json().get("operational_intent_references", [])
        ...
        resp = scd_session.put(
            "/operational_intent_references/{}/{}".format(op_id, existing_op["ovn"]),
            json=req,
            scope=SCOPE_SC,
        )
        assert resp.status_code == 200, resp.content
