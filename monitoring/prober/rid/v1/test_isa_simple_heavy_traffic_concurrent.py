...
async def _put_isa(isa_id, req, session_ridv1):
    async with SEMAPHORE:
        return isa_id, await session_ridv1.put(
            "{}/{}".format(ISA_PATH, isa_id), json=req, scope=Scope.Write
        )


async def _get_isa(isa_id, session_ridv1):
    async with SEMAPHORE:
        return isa_id, await session_ridv1.get(
            "{}/{}".format(ISA_PATH, isa_id), scope=Scope.Read
        )


async def _delete_isa(isa_id, version, session_ridv1):
    async with SEMAPHORE:
        return isa_id, await session_ridv1.delete(
            "{}/{}/{}".format(ISA_PATH, isa_id, version), scope=Scope.Write
        )
...
def test_create_isa_concurrent(ids, session_ridv1_async):
    ...
    results = loop.run_until_complete(
        asyncio.gather(
            *[
                _put_isa(isa_id, req, session_ridv1_async)
                for isa_id in map(ids, ISA_TYPES)
            ]
        )
    )
    for isa_id, resp in results:
        ...
        assert_datetimes_are_equal(
            data["service_area"]["time_end"], req["extents"]["time_end"]
        )
...
def test_get_isa_by_ids_concurrent(ids, session_ridv1_async):
    ...
    results = loop.run_until_complete(
        asyncio.gather(
            *[_get_isa(isa_id, session_ridv1_async) for isa_id in map(ids, ISA_TYPES)]
        )
    )
    for isa_id, resp in results:
        assert resp[0] == 200, resp[2]
...
def test_delete_isa_concurrent(ids, session_ridv1_async):
    ...
    results = loop.run_until_complete(
        asyncio.gather(
            *[
                _delete_isa(isa_id, version_map[isa_id], session_ridv1_async)
                for isa_id in map(ids, ISA_TYPES)
            ]
        )
    )
