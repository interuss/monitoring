...
def test_create_sub(ids, session_ridv2):
    ...
    assert_datetimes_are_equal(
        ...
        req_body["extents"]["time_start"]["value"],
    )
    assert_datetimes_are_equal(
        data["subscription"]["time_end"]["value"],
        req_body["extents"]["time_end"]["value"],
    )
...
def test_get_sub_by_search(ids, session_ridv2):
    ...
    resp = session_ridv2.get(
        "{}?area={}".format(SUBSCRIPTION_PATH, common.GEO_POLYGON_STRING)
    )
    assert resp.status_code == 200, resp.content
...
def test_get_sub_by_searching_huge_area(session_ridv2):
    resp = session_ridv2.get(
        "{}?area={}".format(SUBSCRIPTION_PATH, common.HUGE_GEO_POLYGON_STRING)
    )
...
def test_delete_sub_wrong_version(ids, session_ridv2):
    resp = session_ridv2.delete(
        "{}/{}/fake_version".format(SUBSCRIPTION_PATH, ids(SUB_TYPE))
    )
    assert resp.status_code == 400, resp.content
...
def test_get_sub_with_loop_area(session_ridv2):
    resp = session_ridv2.get(
        "{}?area={}".format(SUBSCRIPTION_PATH, common.LOOP_GEO_POLYGON_STRING)
    )
    assert resp.status_code == 400, resp.content
