"""Run the (context x test) matrix and emit one plot per test.

PYTHONPATH=. python -m monitoring.dss_bench.run --processes 8 --duration 30
"""
...
from monitoring.dss_bench import arms, contexts, environment, measure, plot, tests
from monitoring.dss_bench.config import GlobalConfig
from monitoring.dss_bench.contexts.base import Context
from monitoring.dss_bench.driver import run_test

# Auto-discovered from the contexts/ and tests/ packages.
CONTEXTS = contexts.discover()  # name -> Context subclass
TESTS = tests.discover()  # name -> BenchTest subclass
DEFAULT_CONTEXT = sorted(CONTEXTS)[0] if CONTEXTS else None
DEFAULT_TESTS = sorted(n for n, cls in TESTS.items() if cls.default)


def parse_args() -> tuple[GlobalConfig, list[arms.Arm], Context, list, Path]:
    ...
    p.add_argument(
        ...
        help="Which context to sweep (only one per run).",
    )
    p.add_argument(
        "--tests",
        nargs="+",
    ...
    return cfg, run_arms, context, selected_tests, Path(a.outdir)


def main() -> None:

    cfg, run_arms, context, selected_tests, outdir = parse_args()

    # by_test[test][arm_label] = [point per context variant]
    by_test: dict[str, dict[str, list[dict]]] = {t.name: {} for t in selected_tests}
    ...
    for arm in run_arms:
        ...
        for variant in variants:
            ...
            print(
                ...
                f"[{arm.label}][{context.name}] variant={variant.label} "
                f"image={arm_cfg.dss_image} db={arm_cfg.db_type}"
            )

            environment.down(arm_cfg)
            environment.up(arm_cfg, variant.env)
            targets = environment.dss_targets(arm_cfg)

            for test in selected_tests:
                raw = run_test(test, targets, arm_cfg)

                point = {
                    "label": variant.label,
                    **measure.summarize(raw, arm_cfg.duration_s),
                }
    ...
    (outdir / "results.json").write_text(
        json.dumps({"meta": meta, "by_test": by_test}, indent=2)
    )
    paths = plot.render(by_test, outdir, context.axis_label, meta)
